...
def _sample_on_merl(brdf, theta_h, theta_d, phi_d):
    ...
    evaluate the given BRDF at a given position (theta_h, theta_d, phi_d)
        return:
            ...
            rvectors: Rusinkiewicz xyz coordinate
            brdf_vals: BRDF values
    """

    hx, hy, hz, dx, dy, dz = coords.rangles_to_rvectors(theta_h, theta_d, phi_d)
...
def sample_on_merl(brdf, sampler, nsamples):
    ...
    rangles, rvectors, brdf_vals = _sample_on_merl(brdf, theta_h, theta_d, phi_d)
...
def sample_on_merl_with_rejection(brdf, sampler, nsamples):
    ...
    valid_idx = torch.any(brdf_vals != 0.0, dim=1)
...
class uniform_sampler:
    ...
    def generate(self, nsamples):
        ...
        thphtdpd *= torch.tensor([[PI / 2, PI * 2, PI / 2, PI * 2]])

        th, ph, td, pd = torch.unbind(thphtdpd, dim=1)

        return th.to(device), ph.to(device), td.to(device), pd.to(device)
...
class uniform_sampler_preloaded:
    ...
    def generate(self, nsamples):
        ...
        theta_h, phi_h, theta_d, phi_d = torch.unbind(
            self._loaded[sampled_ind, :], dim=1
        )
...
class inverse_transform_sampler:
    ...
    def generate(self, nsamples):
        ...
        z = torch.multinomial(self.target3D[x, y], 1, replacement=True).flatten()
...
class trainable_sampler_det(nn.Module):
    ...
    def __init__(self, n_fixed, quasi_init=False):
        ...
        self.register_buffer("scale", torch.tensor([PI / 2, PI * 2]), persistent=False)
        ...
        self.factor_h = nn.Parameter(torch.logit(half_init, eps=1e-6))
        self.factor_d = nn.Parameter(torch.logit(diff_init, eps=1e-6))

        self.counter = 0

    def load_samples(self, samples):
        n_samples = len(samples["factor_h"])
        with torch.no_grad():
            self.factor_h[:n_samples, :] = samples["factor_h"]
            self.factor_d[:n_samples, :] = samples["factor_d"]
